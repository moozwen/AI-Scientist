...
# PERFORM WRITEUP
def perform_writeup(
        idea, folder_name, coder, cite_client, cite_model, num_cite_rounds=None,
        engine="semanticscholar"
):
    # 20 citation rounds is the single largest LLM consumer in the pipeline and
    # each round also hits Semantic Scholar. AIS_CITE_ROUNDS=0 skips the stage
    # entirely, which is usually what you want when validating the plumbing.
    if num_cite_rounds is None:
        num_cite_rounds = int(os.environ.get("AIS_CITE_ROUNDS", "20"))
